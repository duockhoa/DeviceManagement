...
import { updateMaintenanceRecord, clearError } from '../../../redux/slice/maintenanceSlice';
...
function EditMaintenanceForm({ handleClose, maintenanceData, onSuccess }) {
    ...
    // State cho hạng mục công việc mặc định (4 loại)
    ...
    useEffect(() => {
        const loadStandards = async () => {
            try {
                ...
                console.error('Error loading checklist standards:', err);
            }
        };
        loadStandards();
    }, []);
    ...
    useEffect(() => {
        ...
        if (maintenanceData) {
            ...
            // Load asset consumables if asset_id exists
            const loadAssetConsumables = async () => {
                if (maintenanceData.asset_id) {
                    try {
                        const assetConsumablesData = await getAssetConsumables(maintenanceData.asset_id);
            ...
            loadAssetConsumables();

            console.log('maintenanceData:', maintenanceData);
            console.log('maintenanceConsumables:', maintenanceData.maintenanceConsumables);
            console.log('spare_parts raw:', maintenanceData.spare_parts);

            // Parse spare_parts safely
            let parsedSpareParts = [];
            if (maintenanceData.spare_parts) {
                try {
                    if (typeof maintenanceData.spare_parts === 'string') {
                        parsedSpareParts = JSON.parse(maintenanceData.spare_parts);
            ...
            setFormData({
                maintenance_code: maintenanceData.maintenance_code || '',
                asset_id: maintenanceData.asset_id || '',
                maintenance_type: maintenanceData.maintenance_type || 'cleaning',
                priority: maintenanceData.priority || 'medium',
                title: maintenanceData.title || '',
                description: maintenanceData.description || '',
                scheduled_date: maintenanceData.scheduled_date ? 
                    new Date(maintenanceData.scheduled_date).toISOString().slice(0, 16) : '',
                ...
                safety_requirements: maintenanceData.safety_requirements || '',
                tools_required: maintenanceData.tools_required || '',
                measuring_tools: maintenanceData.measuring_tools || '',
                safety_tools: maintenanceData.safety_tools || '',
                spare_parts: parsedSpareParts,
                consumables: maintenanceData.maintenanceConsumables ? 
                    maintenanceData.maintenanceConsumables.map(item => {
                        console.log('Consumable item:', item);
                        return {
                            asset_consumable_id: item.asset_consumable_id || '',
                            item_name: item.item_name || (item.assetConsumable ? item.assetConsumable.item_name : ''),
                            specification: item.specification || (item.assetConsumable ? item.assetConsumable.specification : ''),
                            consumable_category_id: item.consumable_category_id,
            ...
            // Load work tasks if exists
            if (maintenanceData.workTasks && maintenanceData.workTasks.length > 0) {
                // Phân loại work tasks thành defaultTasks và customTasks
                const loadedDefaultTasks = {
                    cleaning: { checked: false, assignedTo: [] },
                    inspection: { checked: false, assignedTo: [] },
                    maintenance: { checked: false, assignedTo: [] },
                    corrective: { checked: false, assignedTo: [] }
                ...
                maintenanceData.workTasks.forEach(task => {
                    // Kiểm tra xem có phải default task không (dựa vào task_type)
                    if (task.task_type && ['cleaning', 'inspection', 'maintenance', 'corrective'].includes(task.task_type)) {
                        loadedDefaultTasks[task.task_type] = {
                            checked: true,
                            assignedTo: normalizeAssigned(task.assigned_to)
                        };
                    } else {
                        // Custom task
                        customTasks.push({
                ...
                setDefaultTasks(loadedDefaultTasks);
                setWorkTasks(customTasks);
            }
        }
    }, [dispatch, assets, users, maintenanceData]);
    
    // Auto-check default task khi chọn loại bảo trì (chỉ khi tạo mới, không áp dụng khi edit)
    useEffect(() => {
        // Auto chọn công việc chính theo maintenance_type (áp dụng cả khi có dữ liệu mới thay đổi loại)
        if (formData.maintenance_type) {
            setDefaultTasks(prev => ({
                cleaning: { ...prev.cleaning, checked: formData.maintenance_type === 'cleaning' },
                inspection: { ...prev.inspection, checked: formData.maintenance_type === 'inspection' },
                maintenance: { ...prev.maintenance, checked: formData.maintenance_type === 'maintenance' || formData.maintenance_type === 'preventive' },
                corrective: { ...prev.corrective, checked: formData.maintenance_type === 'corrective' }
            }));
        }
    ...
    const updateConsumable = (index, field, value) => {
        setFormData(prev => ({
            ...
            consumables: prev.consumables.map((item, i) => {
                ...
                // Auto-fill when selecting asset_consumable_id
                ...
                const total_cost = field === 'quantity_required' || field === 'unit_cost'
    ...
    // Work tasks management functions
    const addWorkTask = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const { maintenance_code, ...updateData } = formData;
            
            // Calculate estimated cost from spare_parts and consumables
            ...
            const submitData = {
                ...
                workTasks: (() => {
                    ...
                    if (defaultTasks.cleaning.checked) {
                        items.push({
                            task_name: 'Vệ sinh',
                            task_type: 'cleaning',
                    ...
                    if (defaultTasks.inspection.checked) {
                        items.push({
                            task_name: 'Kiểm tra',
                    ...
                    if (defaultTasks.maintenance.checked) {
                        items.push({
                            task_name: 'Bảo trì',
                    ...
                    if (defaultTasks.corrective.checked) {
                        items.push({
                            task_name: 'Sửa chữa',
                            task_type: 'corrective',
                            description: 'Sửa chữa / khắc phục sự cố',
                            assigned_to: defaultTasks.corrective.assignedTo,
                            status: 'pending',
                            order_index: orderIndex++
                        });
                    }
            ...
            console.log('Edit form data before submit:', formData); // Debug log
            ...
            setSuccessMessage('Cập nhật lịch bảo trì thành công!');
            setShowSuccess(true);
        ...
        <Box sx={{ width: "100%", height: "95vh", display: 'flex', flexDirection: 'column', backgroundColor: '#f0f7ff' }}>
            ...
            <Stack sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
                ...
                <Box sx={{ flex: 1, display: 'flex', backgroundColor: '#f5f5f5' }}>
                                ...
                                <Grid2 container spacing={3}>
                                    ...
                                    <Grid2 xs={12}>
                                        ...
                                        <InputField
                                            ...
                                            multiline
                                            rows={3}
                                            placeholder="📌 Danh sách công cụ chính cần thiết:&#10;- Tuốc nơ vít đa năng&#10;- Kìm điện&#10;- Máy đo điện đa năng&#10;- Bộ khóa lục giác..."
                                            fullWidth
                                        />
                                    </Grid2>
                                    ...
                                    <Grid2 xs={12} md={6}>
                                        ...
                                        <InputField
                                            ...
                                            multiline
                                            rows={3}
                                            placeholder="🔍 Thiết bị đo lường cần thiết:&#10;- Đồng hồ đo điện áp&#10;- Thiết bị đo nhiệt độ&#10;- Máy đo độ rung..."
                                            fullWidth
                                        />
                                    </Grid2>
                            ...
                            <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                                ...
                                <Stack direction="row" spacing={1}>
                                    ...
                                    <Button
                                        ...
                                        sx={{ minWidth: 180, backgroundColor: '#1976d2' }}
                                    >
                                        Áp dụng tiêu chuẩn
                                    </Button>
                                </Stack>
                    ...
                    {/* Công việc chính theo loại bảo trì (hiển thị duy nhất loại tương ứng) */}
                    <Box sx={{ mb: 3, p: 2, bgcolor: '#f5f5f5', borderRadius: 1 }}>
                        <Typography variant="h6" sx={{ fontWeight: 'bold', mb: 2 }}>
                            Công việc chính (theo loại bảo trì)
                        </Typography>

                        {defaultTasks.cleaning.checked && (
                            <Box sx={{ mb: 2, p: 2, bgcolor: 'white', borderRadius: 1 }}>
                                <Typography variant="body1" fontWeight="bold">Vệ sinh</Typography>
                                <Box sx={{ ml: 2, mt: 1 }}>
                                    <Autocomplete
                                        multiple
                                        size="small"
                        ...
                        {defaultTasks.inspection.checked && (
                            <Box sx={{ mb: 2, p: 2, bgcolor: 'white', borderRadius: 1 }}>
                                <Typography variant="body1" fontWeight="bold">Kiểm tra</Typography>
                                <Box sx={{ ml: 2, mt: 1 }}>
                                    <Autocomplete
                                        multiple
                                        size="small"
                        ...
                        {defaultTasks.maintenance.checked && (
                            <Box sx={{ mb: 2, p: 2, bgcolor: 'white', borderRadius: 1 }}>
                                <Typography variant="body1" fontWeight="bold">Bảo trì</Typography>
                                <Box sx={{ ml: 2, mt: 1 }}>
                                    <Autocomplete
                                        multiple
                                        size="small"
                        ...
                        {defaultTasks.corrective.checked && (
                            <Box sx={{ mb: 2, p: 2, bgcolor: 'white', borderRadius: 1 }}>
                                <Typography variant="body1" fontWeight="bold">Sửa chữa</Typography>
                                <Box sx={{ ml: 2, mt: 1 }}>
                                    <Autocomplete
                                        multiple
                                        size="small"
            ...
            {/* Dialog chọn tiêu chuẩn checklist */}
            <Dialog open={openStandardDialog} onClose={() => setOpenStandardDialog(false)} maxWidth="md" fullWidth>
                <DialogTitle>Áp dụng tiêu chuẩn checklist</DialogTitle>
                <DialogContent dividers>
                    <Stack spacing={2}>
                        <Typography variant="body2">
                            Chọn một tiêu chuẩn để tự động thêm các hạng mục checklist vào lệnh bảo trì.
                        </Typography>
                        <TextField
                            select
                            label="Tiêu chuẩn checklist"
                            size="small"
